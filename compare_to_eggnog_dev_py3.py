...
def get_eggnog_splits(e_clusters, m_clusters, f_name):
    ...
    for clu_name, members in m_clusters.items():
        for seq in members:
            seq2mcluster[seq].append(clu_name)

    print (len(seq2mcluster), 'seqs in m_clusters')
    ...
    for e_clu, e_members in e_clusters.items():
        ...
        for m_clu in related_mmseqs_clusters:
            ...
            missing_seqs = (e_members - m_members) - genes_to_be_excluded
            missings = len(missing_seqs)
            ...
            if best_match_missing is None or missings < best_match_missing: 
                ...
                best_match = m_clu
                best_missing_seqs = missing_seqs
                
            if missings == 0: 
                percent = (float(missings)/float((len(e_members))))
                break

        missing_sp = set([s.split(".")[0] for s in best_missing_seqs])
        mmseqs_seqs= m_clusters[m_clu]
        mmseq_sp = set([s.split(".")[0] for s in mmseqs_seqs])
        
        print('\t'.join(map(str, [e_clu, best_match, len(e_members), len(m_clusters[best_match]), best_match_missing, percent,len(related_mmseqs_clusters),len(egg_sp),len(mmseq_sp),len(missing_sp),','.join(related_mmseqs_clusters)])),file = RESULTS_F)
...
def compared_clusters(e_clusters, m_clusters, f_name):
    ...
    # sequence name (what clusters)
    seq2ecluster = defaultdict(list)
    for clu_name, members in e_clusters.items():
        for seq in members:
            seq2ecluster[seq].append(clu_name)
    ...
    for m_clu, m_members in m_clusters.items():
        ...
        if best_match != None:
            ...
            print ("\t".join(map(str, (m_clu, len(related_eggnog_clusters), e_clu, len(m_members), len(e_clusters[e_clu]),common, m_missing, e_missing, m_missing_percent, e_missing_percent))), file = RESULTS_F)
...
#eggnog_cls = "/scratch/pipeline_mmseqs/egg5/results/clusters_list/e5.luca_members.tsv"
